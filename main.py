...
def insert_voters(conn, cur, voter):
    try:
        cur.execute("""INSERT INTO voters (voter_id, voter_name, date_of_birth, gender, nationality, registration_number, address_street, address_city, address_state, address_country, address_postcode, email, phone_number, cell_number, picture, registered_age)
        ...
        logger.error(f"Error inserting voter: {e}")
        conn.rollback()
    ...
    try:
        ...
        if len(candidates) == 0:
            for i in range(3):
                ...
                if candidate:
                    cur.execute("""INSERT INTO candidates (candidate_id, candidate_name, party_affiliation, biography, campaign_platform, photo_url)
                                    VALUES (%s, %s, %s, %s, %s, %s)""",
        ...
        for i in range(1000):
            ...
            if voter_data:
                ...
                logger.info(f'Produced voter {i}, data: {voter_data}')
                producer.flush()
